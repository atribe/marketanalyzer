...
public class Ibd50TrackingServiceImpl implements Ibd50TrackingService{
	...
	public Ibd50Tracking findByActiveAndTicker(boolean active, TickerSymbol ticker) throws Ibd50NotFound, Ibd50TooManyFound {
		...
		if( foundTickerSymbol == null ) {
			throw new Ibd50NotFound("By Active and Ticker search for " + ticker.getSymbol() + " failed because the Ticker was not found in the Ticker DB.");
		}
		
		List<Ibd50Tracking> trackingList = ibd50TrackingRepo.findByActiveAndTicker(active, foundTickerSymbol);
		
		if(trackingList.isEmpty()) {
			throw new Ibd50NotFound("The Ticker '" + foundTickerSymbol.getSymbol() + "' was not found with the active state '" + active + "'.");
