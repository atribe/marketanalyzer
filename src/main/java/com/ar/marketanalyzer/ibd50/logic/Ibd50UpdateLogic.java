...
public class Ibd50UpdateLogic {
    ...
    private boolean isDbUpToDate() {
        ...
        LocalDateTime previousMonday = LocalDateTime.now()
                                                    .with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY));   // Get the most recent monday

        List<Ibd50Rank> newestRankings;
        try {
            // Check the db for any rankings modified after monday
            newestRankings = rankingService.findByModificationTimeAfter(previousMonday);
        ...
        if (newestRanking.getLocalDateModificationTime().isAfter(previousMonday) || newestRanking.getLocalDateRankDate()
                                                                                                 .equals(previousMonday)) {                            // Doing a check with the LocalDate to make sure it is after monday
    ...
    private void deactivateOldTrackers(List<Ibd50Rank> webIbd50) {
        ...
        for (Ibd50Tracking tracker : activeTrackers) {                                        // Loop through the active trackers
            boolean
                    trackerStillActive
                    = false;                                                // Start with the assumption that the tracker is now inactive
            ...
            if (!trackerStillActive) {                                                        // After all the new ranks are checked against the current tracker and the tracker isn't active
                ...
                try {
                    leaveOhlcv = ohlcvService.findByTickerAndDate(tracker.getTicker(), tracker.newMonday());
    ...
    private void addThisWeeksListToDB(List<Ibd50Rank> webIbd50) {
        for (Ibd50Rank rankingRow : webIbd50) {                                        // Cycle through the each of row of the top 50
            ...
            Symbol
                    foundTicker
                    = tsService.createOrFindDuplicate(rowTicker);    // Adding the symbol to the DB or getting the one in the db out
    ...
    private void runOhlcvUpdate(Symbol symbol) {
        ...
        //set the oldestDate as today
        LocalDateTime ohlcvDesiredStartDate = LocalDateTime.now().minusMonths(monthsOfData);
        LocalDateTime startDate;

        try {
            currentOhlcvList = ohlcvService.findByTickerAndDateAfter(symbol,
                                                                     ohlcvDesiredStartDate);    //see if there is anything in the db after 6 months ago from today
    ...
    private void updateOHLCVforInactiveTracker() {
        ...
        LocalDate inactiveUpdateWindow = LocalDate.now().minusMonths(inactiveUpdateMonthsWindow);
