...
public class RuleSellDDaysAndChurnDays extends AbstractRule {
    ...
    private void runChurnDayAnalysis() {
        SortedMap<LocalDateTime, SecuritiesOhlcv> ohlcvData = currentModel.getOhlcvData();
        SortedMap<LocalDateTime, FollowThruStats> stats = currentModel.getStats();
    ...
    private void countDDaysInWindow() {
        ...
        ArrayList<LocalDateTime> keys = new ArrayList<>(ruleResult.keySet());
        for (int i = keys.size() - 1; i > 0; i--) {
            ...
            for (int j = i;
                 j > i - ddayWindow && j > 0;
                 j--) { //This loop starts at i and then goes back dDayWindow days adding up all the d days
