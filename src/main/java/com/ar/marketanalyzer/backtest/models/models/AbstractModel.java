...
import com.ar.marketanalyzer.common.properties.DefaultConstants;
...
public abstract class AbstractModel extends PersistableEntityInt {
    ...
    protected List<AbstractRule> ruleList = new ArrayList<>();

    @Transient
    protected SortedMap<LocalDateTime, BuySellTrigger> buySellTriggers = new TreeMap<>();
    ...
    @OneToMany(mappedBy = "model", cascade = CascadeType.ALL, fetch = FetchType.LAZY)
    @OrderBy("date")
    protected SortedSet<DollarValue> valueSet = new TreeSet<>();

    @Transient
    protected SortedMap<LocalDateTime, DollarValue> valueMap = new TreeMap<>();
     ...
     */
    @Transient
    protected SortedMap<LocalDateTime, SecuritiesOhlcv> ohlcvData = new TreeMap<>();
    @Transient
    protected SortedMap<LocalDateTime, Stats> defaultStats = new TreeMap<>();

    protected static final int modelMonthRange = DefaultConstants.DEFAULT_MONTHS_OF_DATA;
    protected static final BigDecimal defaultInitialInvestment = new BigDecimal(1000);
    protected static final LocalDateTime defaultStartDate = LocalDateTime.now().minusMonths(modelMonthRange);
    protected static final LocalDateTime defaultEndDate = LocalDateTime.now();
    ...
    public AbstractModel(Symbol symbol, LocalDateTime startDate) {
        this(symbol, defaultStartDate, defaultEndDate);
    }

    public AbstractModel(Symbol symbol, LocalDateTime startDate, LocalDateTime endDate) {
        this.symbol = symbol;
    ...
    private void makeTrades() {
        ...
        Set<LocalDateTime> keys = buySellTriggers.keySet();
        for (LocalDateTime key : keys) {
            if (buySellTriggers.containsKey(key)) {
                ...
                if (trigger.isSell()
                        && trade.getBuyDate()
                        != null) {                                        //if today is a sell day and the buy date is set
                    trade.sell(trigger.getDate(), ohlcvData.get(key).getClose());                                        //set sell price
                } else if (trigger.isBuy()
                        && trade.getBuyDate() == null
                        && trade.getSellDate() == null) {    //if today is a buy day, and buy and sell dates are not set
    ...
    public void setBuySellTriggers() {
        ...
        for (AbstractRule rule : ruleList) {                                //Loop through the rules
            SortedMap<LocalDateTime, AbstractRuleResult> results = rule.getRuleResult();

            for (Map.Entry<LocalDateTime, AbstractRuleResult> resultEntry : results.entrySet()) {                    //Loop through the results of the rule
    ...
    private void calculateDollarValue() {
        ...
        for (int i = 0; i < keys.size(); i++) {
            ...
            if (!ohlcvData.firstKey().equals(currentOhlcv.getDate())) {                    // skip day 1
                DollarValue
                        currentValue
                        = valueMap.get(keys.get(i));                        // getting today's value (will be calculated below
                DollarValue previousValue = valueMap.get(keys.get(i - 1));                    // get yesterday's value

                if (tradeOpen && !currentTrade.getBuyDate()
                                              .equals(currentOhlcv.getDate())) {    // if a trade has been opened previously, but not signaled to buy today
                    ...
                    if (currentTrade.getBuyDate()
                                    .equals(previousOhlcv.getDate())) {        // if the signal to buy came yesterday, then you bought at the open of today
                        percentChange = currentOhlcv.getClose().subtract(currentOhlcv.getOpen()).doubleValue() / currentOhlcv.getOpen()
                                                                                                                             .doubleValue();
                    } else {                                                                // if the trade is already open monitor the change from yestreday's close to today's close
                        percentChange = currentOhlcv.getClose().subtract(previousOhlcv.getClose()).doubleValue() / previousOhlcv.getClose()
                                                                                                                                .doubleValue();
                    }
                    ...
                    percentChange = currentOhlcv.getOpen().subtract(previousOhlcv.getClose()).doubleValue() / previousOhlcv.getClose()
                                                                                                                           .doubleValue();
                    currentValue.setDollarValue(new BigDecimal(previousValue.getDollarValue().doubleValue() * (1 + percentChange)));
                } else {
                ...
                if (currentTrade.getSellDate()
                                .equals(currentOhlcv.getDate())) {                    // check for sell date at the end, because you wouldn't know until end of day, which means you'll sell tomorrow morning
                    tradeOpen = false;
                    tradeIterator++;
    ...
    public void calcStats() {
        ...
        for (int i = keys.size() - 1; i >= 0; i--) {
            ...
            for (int j = i;
                 j > i - loopDays && j >= 0;
                 j--) { //This loop starts at i and then goes back loopDays days adding up all the d days
            ...
            Stats stat = new Stats(ohlcvData.get(keys.get(i)).getDate(), close50DayAvg, close100DayAvg, close200DayAvg, vol50DayAvg);
            defaultStats.put(keys.get(i), stat);
        }
    }

    public void convertValueMapToSet() {
        for (Map.Entry<LocalDateTime, DollarValue> valueEntry : valueMap.entrySet()) {
    ...
    public SortedMap<LocalDateTime, SecuritiesOhlcv> getOhlcvData() {
        return ohlcvData;
    }
    ...
    public void setOhlcvData(SortedMap<LocalDateTime, SecuritiesOhlcv> ohlcvData) {
        this.ohlcvData = ohlcvData;
    }

    public abstract <T> TreeMap<LocalDateTime, T> getStats();
    ...
    public SortedMap<LocalDateTime, DollarValue> getValueList() {
        return valueMap;
    }
    ...
    public SortedMap<LocalDateTime, DollarValue> getValueMap() {
        return valueMap;
    }

    public void setValueMap(SortedMap<LocalDateTime, DollarValue> valueMap) {
